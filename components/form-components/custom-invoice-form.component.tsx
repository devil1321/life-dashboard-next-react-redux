...
const CustomForm:React.FC<CustomFormProps> = ({customFormFn}) => {
  ...
  const handleCustomForm = (e:any) => {
    e.preventDefault()
    let formData = {}
    let tempFields:any = []
    const allFields = document.querySelectorAll('.invoices__field input') as NodeListOf<HTMLInputElement>
    allFields.forEach((field:HTMLInputElement)=>{
      const fieldData = {
        [field.name]:field.value
      }
      if(field.name !== 'field-name' && field.name !== 'field-text'){
